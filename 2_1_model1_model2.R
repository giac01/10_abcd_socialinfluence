...
modelarg =  c(
  list(
    formula = sit_values_finalrating2 ~ 1 + sit_values_initialrating2 + (sit_values_delta) + (1 + sit_values_delta | subject),
    seed = 3
  ),
  brm_args
)
...
# Does scenario and trial number moderate the social infleunce effect ----------
...
modelarg =  c(
  list(
    ...
    init = list(
      list(b = rep(0, 2), Intercept = 0, Intercept_sigma = .01, sd_1 = .5, sd_2 = .5,  sd_3 = .5 )
    )
  ),
  brm_args
)
